...
function testExampleHttpUrlsSkipsInlineCodeSpans() {
    ...
    const result = createWriter([]).__example_http_urls(markdown);

    assert.equal(result, markdown);
}

async function testCalloutPreservesNestedMarkdownLists() {
    const callout = {
        block_id: 'callout',
        block_type: 19,
    ...
    const blocks = [
        ...
        textBlock('free-url', 'free', [
            textRun('https://{cluster-id}.serverless.{region}.vectordb.zillizcloud.com', { inline_code: true }),
        ]),
        bulletBlock('dedicated', 'managed', [textRun('Dedicated', { bold: true })], ['dedicated-url']),
        textBlock('dedicated-url', 'dedicated', [
            textRun('https://{cluster-id}.{region}.vectordb.zillizcloud.com:19530', { inline_code: true }),
        ]),
        ...
        textBlock('external-url', 'external', [
            textRun('https://{project-id}.{region}.api.zillizcloud.com', { inline_code: true }),
        ]),
    ];
    ...
    assert.doesNotMatch(markdown, /<ul>|<p>|<li>/);

    await assertMdxCompiles(markdown);
}

async function testQuotePreservesNestedMarkdownLists() {
    const quote = {
        block_id: 'quote',
        block_type: 34,
...
async function run() {
    ...
    await testCalloutPreservesNestedMarkdownLists();
    await testQuotePreservesNestedMarkdownLists();
