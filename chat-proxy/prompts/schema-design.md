  ...
  - If the embedding model changes to a different output dimension, do not imply the existing vector field dimension can be modified in place. Recommend creating a compatible collection or migration path, rebuilding indexes, and re-embedding/reingesting data.
  ...
  - implying dynamic fields can fix a declared vector field dimension mismatch
  - recommending destructive collection drops as the default migration path without mentioning backup, parallel collection, aliases, or planned cutover
