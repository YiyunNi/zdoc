...
- Do not rank Cohere versus Voyage AI by quality unless the provided context documents that comparison. Recommend choosing by provider availability, model requirements, language/document-length needs, credential policy, cost, latency, and benchmark results on the user's data.
...
- In Cohere-versus-Voyage answers, do not fill comparison-table cells with unsupported labels such as `Primarily English`, `Multilingual`, `Handles typical lengths`, `Optimized for long documents`, or similar comparative claims unless the provided context states them. Use neutral wording such as "depends on the selected provider model; verify in provider docs and benchmark."
...
- For reranking latency and cost questions, do not include code unless the user asks for implementation. Do not say Boost, Decay, RRF, or Weighted rankers add a specific latency such as "<10ms" or "minimal latency"; say they avoid an external model call but still need workload measurement.
