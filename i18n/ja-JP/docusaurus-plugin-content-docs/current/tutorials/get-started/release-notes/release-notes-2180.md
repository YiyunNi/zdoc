...
beta: FALSE
notebook: FALSE
description: "このリリースでは、Zilliz Cloud は運用効率、柔軟性、ユーザーエクスペリエンスの向上を目的としたいくつかの強力な機能強化を導入しました。これには、クラスターレベルのスケジュールされた自動スケーリングのサポート、新しい Merge Data API によるスキーマの進化、データ取り込みを合理化するクラウドネイティブなデータ層である stage の導入、クロスデータベース選択によるクラスターレベルのバックアップからの部分的な復元、および JSON Path インデックスの UI サポートが含まれます。これらの機能を組み合わせることで、ユーザーは複雑なワークロードをより効果的に管理し、メンテナンスのオーバーヘッドを削減し、GenAI 時代における開発サイクルを加速できます。 | Cloud"
type: origin
