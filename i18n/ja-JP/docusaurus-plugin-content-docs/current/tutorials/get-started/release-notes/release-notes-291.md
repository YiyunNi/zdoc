...
beta: FALSE
notebook: FALSE
description: "今回のアップデートでは、Zilliz Cloud が Milvus の新しい RESTful API V2 をサポートし、一貫したインターフェースと拡張された機能を提供します。新しいドキュメントチャットボットにより、ユーザーサポートが強化されました。ジョブセンターでは、バックアップ、復元、移行、インポート、コレクションのクローンなどのタスクを管理・追跡するための直感的なインターフェースが導入されました。専用クラスター向けのオートスケーリング（プライベートプレビュー版）は、コンピューティングユニット（CU）容量しきい値によってトリガーされ、需要に応じて容量を動的に調整します。その他の機能強化には、クラスター監視メトリクスの追加、クラスター管理インターフェースのリニューアル、ユーザー向け電子メールテンプレートの改善が含まれます。 | Cloud"
type: origin
