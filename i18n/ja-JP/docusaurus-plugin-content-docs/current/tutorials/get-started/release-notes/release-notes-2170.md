...
beta: FALSE
notebook: FALSE
description: "このリリースでは、Zilliz Cloud の複数の機能にわたり、より洗練された直感的なユーザーエクスペリエンスを実現しました。再設計された移行コンソールからポリシーベースのアラート、改善された mmap 制御まで、ワークフローをより高速で柔軟かつ管理しやすくすることに注力しました。新しい AI アシスタント機能と GCP での BYOC サポートにより、インフラストラクチャの管理、環境の監視、サポートの要否にかかわらず、プラットフォームの能力と使いやすさがさらに拡張されました。 | Cloud"
type: origin
