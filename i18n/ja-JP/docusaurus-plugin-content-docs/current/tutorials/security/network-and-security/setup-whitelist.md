...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud のクラスタ IP 許可リストは、プロジェクトレベルで強固なセキュリティ層として機能し、指定したプロジェクト内のすべてのクラスタにその利点を拡張します。IP 許可リストを実装することで、プロジェクトのクラスタへのアクセスを特定の IP アドレスのグループに限定でき、悪意のある攻撃のリスクを大幅に軽減できます。"
type: origin
