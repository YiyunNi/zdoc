...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、デフォルトで 256 ビット Advanced Encryption Standard (AES-256) アルゴリズムを使用して、ディスクまたはオブジェクトストレージ上の保存データを暗号化します。最高レベルのセキュリティ要件を持つ顧客向けに、Zilliz Cloud は顧客のクラウドプロバイダーの Key Management Service (KMS) と Zilliz Cloud の Customer-Managed Encryption Key (CMEK) 機能を組み合わせることで、追加のセキュリティ層を提供します。 | Cloud"
type: origin
