...
beta: FALSE
notebook: FALSE
description: "このトピックでは、OpenID Connect (OIDC) プロトコルを使用して Okta でシングルサインオン (SSO) を設定する方法について説明します。 | Cloud"
type: origin
...
1. 次の設定で新しい Web アプリ統合を設定します。
    ...
    - **Sign-in redirect URIs**: [ステップ1](./openid-connect#step-1-access-sp-details-in-zilliz-cloud-console)で Zilliz Cloud コンソールからコピーした**Single sign-on URL** をここに貼り付けます。
...
1. アプリの詳細ページで、次の情報を取得します。
    ...
    これらの値は、[ステップ3](./openid-connect#step-3-configure-idp-settings-in-zilliz-cloud-console)で Zilliz Cloud コンソールに必要になります。
