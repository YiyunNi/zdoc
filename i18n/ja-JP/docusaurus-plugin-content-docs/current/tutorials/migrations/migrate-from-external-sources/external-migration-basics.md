...
beta: FALSE
notebook: FALSE
description: "外部移行機能により、ベクトルデータベースや検索システムを Zilliz Cloud へ移行するプロセスが簡素化されます。Pinecone や Qdrant などのベクトルデータベースから、Elasticsearch や OpenSearch などのベクトル対応検索エンジンまで、Zilliz Cloud はデータの整合性を保ちつつ移行の複雑さを最小限に抑えるための移行ツールを提供します。 | Cloud"
type: origin
