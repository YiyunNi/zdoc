...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、多様なビジネスニーズに対応するため、さまざまなクラスターデプロイメントオプションを提供します。| Cloud"
type: origin
...
Zilliz Cloud では、さまざまなビジネスニーズに対応できるよう、複数のクラスターデプロイオプションを提供しています。

- **Free**: ストレージ容量、vCU 消費量、コレクション数に制限があるため、学習や個人プロジェクトの開始点として利用できます。

- **Serverless**: ワークロードに応じて自動的にスケーリングする共有環境を提供します。リソースを事前にプロビジョニングする必要がなく、予測不能または急激に変動するトラフィックに対して優れたコスト効率と弾力性を実現します。

- **Dedicated**: 一貫性があり予測可能なパフォーマンスを必要とする本番ワークロード向けに、分離された専用環境を提供します。このオプションは、持続的な高スループットおよびレイテンシーに敏感なアプリケーションに最適です。
...
- Zilliz Cloud への登録済みであること。手順については、[登録 with Zilliz Cloud](./register-with-zilliz-cloud) を参照してください。

- クラスターを作成する組織またはプロジェクトのオーナーであること。ロールと権限の詳細については、[Access Control](./access-control) を参照してください。

## フリークラスターの作成\{#create-a-free-cluster}

<Admonition type="info" icon="📘" title="Notes">

<p>各組織につき、フリークラスターは 1 つしか作成できません。追加のクラスターが必要な場合は、Serverless または Dedicated を選択してください。</p>
...
以下のデモでは、**Free** クラスターの作成方法を示します。

<Supademo id="cmhixdror61dofati1xmaai6j?utm_source=link" title=""  />

クラスター作成中には、一度だけ表示されるクラスターの認証情報（ユーザー名とパスワード）を必ず保存してください。

クラスターのステータスが「Running」に変わると、クラスターが正常に作成されたことを意味します。その後、クラスターエンドポイントとトークンをコピーし、それらを使ってクラスターに[接続](./connect-to-cluster)できます。

</TabItem>

<TabItem value="Bash">

リクエストは以下の例のように記述します。ここで `{API_KEY}` は認証に使用する API キーです。

次の `POST` リクエストはリクエストボディを受け取り、ID が `proj-xxxxxxxxxxxxxxxxxxxxx` のプロジェクト内に `cluster-free` という名前のフリークラスターを作成します。
