...
beta: FALSE
notebook: FALSE
description: "地域をまたがる時間を追跡するアプリケーション（例：eコマースシステム、コラボレーションツール、分散ログ）では、タイムゾーン付きタイムスタンプの正確な処理が必要です。Zilliz Cloud の `TIMESTAMPTZ` データ型は、タイムゾーン情報とともにタイムスタンプを保存することで、この機能を提供します。"
type: origin
