...
beta: FALSE
notebook: FALSE
description: "JSON シュレッディングは、従来の行ベースのストレージを最適化された列ベースのストレージに変換することで、JSON クエリを高速化します。データモデリングにおける JSON の柔軟性を維持しつつ、Zilliz Cloud は裏側で列ベースの最適化を実行し、アクセスおよびクエリの効率を劇的に向上させます。| Cloud"
type: origin
