...
slug: /json-indexing
sidebar_label: "JSON インデックス"
beta: FALSE
notebook: FALSE
description: "JSON フィールドは、Zilliz Cloud に構造化されたメタデータを保存するための柔軟な方法を提供します。インデックスなしでは、JSON フィールドに対するクエリはコレクション全体のスキャンを必要とし、データセットが大きくなるにつれて遅くなります。JSON インデックスは、JSON データ内にインデックスを作成することで高速なルックアップを可能にします。 | Cloud"
type: origin
...
- **オプションの型変換**: 必要に応じて、インデックス作成中にデータを変換します

JSONフィールドをインデックス化するための構文は次のとおりです。
...
<table>
   ...
   <tr>
     ...
     <td><p>値を解釈およびインデックス作成する際に使用するデータ型。キーの実際のデータ型と一致する必要があります。</p><p>利用可能なキャストタイプのリストについては、<a href="./json-indexing#supported-cast-types">サポートされているキャストタイプ</a>を参照してください。</p></td>
...
JSONフィールドキーに誤った形式の値が含まれている場合（例: 文字列として保存されている数値）、`json_cast_function`引数にキャスト関数を渡すことで、インデックス作成時にこれらの値を変換できます。
...
<p>変換が失敗した場合（例: 非数値文字列）、その値はスキップされ、インデックス化されません。</p>
...
上記の[サンプルJSON構造](./json-indexing#sample-json-structure)の場合、`metadata`オブジェクト全体をインデックスします。
