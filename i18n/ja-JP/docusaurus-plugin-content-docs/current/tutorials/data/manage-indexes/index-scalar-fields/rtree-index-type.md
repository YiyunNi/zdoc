...
beta: FALSE
notebook: FALSE
description: "`RTREE` インデックスは、Zilliz Cloud において `GEOMETRY` フィールドに対するクエリを高速化するツリーベースのデータ構造です。ポイント、ライン、ポリゴンなどの幾何オブジェクトを Well-known text (WKT) 形式で保存し、空間フィルタリングを高速化したい場合、`RTREE` は理想的な選択肢です。| Cloud"
type: origin
