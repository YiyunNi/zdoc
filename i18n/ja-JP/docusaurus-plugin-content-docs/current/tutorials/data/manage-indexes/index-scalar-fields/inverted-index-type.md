...
beta: FALSE
notebook: FALSE
description: "データに対して頻繁にフィルタークエリを実行する必要がある場合、`INVERTED` インデックスを使用することでクエリパフォーマンスを劇的に向上させることができます。すべてのドキュメントをスキャンする代わりに、Zilliz Cloud は転置インデックスを使用して、フィルター条件に一致する正確なレコードを迅速に特定します。 | Cloud"
type: origin
