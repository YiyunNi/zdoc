...
sidebar_key: "data/manage-indexes/index-vector-fields"
---
...
このガイドでは、コレクション内のベクトルフィールドに対してインデックスを作成・管理する基本的な操作を説明します。

## 概要\{#overview}

Zilliz Cloud は、インデックスファイルに保存されたメタデータを活用して、データを特殊な構造で整理し、検索やクエリ時に要求された情報を高速に取得できるようにします。

Zilliz Cloud は効率的な類似検索を実現するために [AUTOINDEX](./autoindex-explained) を採用しています。また、ベクトル埋め込み間の距離を測定するための以下の[メトリックタイプ](./search-metrics-explained)も提供しています：**コサイン類似度**（COSINE）、**ユークリッド距離**（L2）、**内積**（IP）、**JACCARD**、および **HAMMING**。ベクトルフィールドのタイプとメトリックに関する詳細は、[メトリックタイプs](./search-metrics-explained) および [Schema Explained](./schema-explained) を参照してください。

頻繁にアクセスされるベクトルフィールドおよびスカラーフィールドの両方にインデックスを作成することを推奨します。コレクションに複数のベクトルフィールドが含まれる場合、各ベクトルフィールドに対して個別にインデックスを作成できます。

## 準備\{#preparations}

[Create Collection](./manage-collections-sdks) で説明されているように、コレクション作成リクエストで以下のいずれかの条件が指定されている場合、Zilliz Cloud はコレクション作成時に自動的にインデックスを生成し、メモリにロードします。

- ベクトルフィールドの次元数とメトリックタイプ、または
- スキーマとインデックスパラメータ。

以下のコードスニペットでは、既存のコードを再利用して Zilliz Cloud への接続を確立し、インデックスパラメータを指定せずにコレクションを作成します。この場合、コレクションにはインデックスが存在せず、ロードもされていません。
