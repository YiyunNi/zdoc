...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud のハイライターは、テキストフィールド内の一致した用語をカスタマイズ可能なタグで囲むことで注釈を付けます。ハイライト機能は、ドキュメントが一致した理由を説明し、結果の可読性を向上させ、検索および RAG アプリケーションでのリッチなレンダリングをサポートします。"
type: origin
