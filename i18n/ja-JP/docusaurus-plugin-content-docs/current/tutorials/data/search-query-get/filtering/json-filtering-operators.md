...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、JSON フィールドのクエリとフィルタリングに対応した高度な演算子をサポートしており、複雑な構造化データの管理に最適です。これらの演算子により、JSON ドキュメント内の特定の要素、値、または条件に基づいてエンティティを取得する、非常に効果的なクエリが可能になります。このセクションでは、Zilliz Cloud で JSON 固有の演算子を使用する方法を、その機能を説明する実践的な例とともに解説します。 | Cloud"
type: origin
