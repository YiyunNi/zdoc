...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud では、検索リコールとパフォーマンスのバランスを取るために `level` という検索パラメータを導入しています。また、現在の検索の推定リコール率をユーザーに提供するための別の検索パラメータ `enablerecallcalculation` も提供しています。これら2つのパラメータを組み合わせて、ベクトル検索のリコール率を調整できます。"
type: origin
