...
<p>この機能は、Zilliz Cloud 上でホストされているハイライトモデルに依存しています。詳細については、<a href="./hosted-models#supported-models">ホストモデル</a>を参照してください。</p>
...
ハイライト ステージ は、Zilliz Cloud にデプロイされたホストされた [ハイライトモデル](./hosted-models#supported-models) によって実現されます。このモデルは、取得したドキュメントを処理し、クエリに対してセマンティックに関連するテキストセグメントを特定します。
...
- **ハイライトモデルのデプロイメント**
    ...
    利用可能なハイライトモデルとデプロイメント手順については、[Hosted Models](./hosted-models) を参照してください。
...
- **セマンティック検索用の埋め込みモデル**
    ...
    **オプション 1: モデルベースの埋め込み関数（推奨）**
    ...
    - **ホスト型埋め込みモデル**: Zilliz Cloud を介してホスト型埋め込みモデルをデプロイし、その `model_deployment_id` を取得します。利用可能なモデルとデプロイメント手順については、[Hosted Models](./hosted-models) を参照してください。
