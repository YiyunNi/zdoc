---
title: "コレクションのTTLを設定する | Cloud"
slug: /set-collection-ttl
sidebar_label: "コレクションのTTLを設定する"
beta: FALSE
notebook: FALSE
description: "データは一度コレクションに挿入されると、デフォルトではそこに残り続けます。しかし、特定のシナリオでは、一定期間後にデータを削除またはクリーンアップしたい場合があります。そのような場合、コレクションのTime-to-Live (TTL) プロパティを設定することで、TTLの期限が切れるとZilliz Cloudが自動的にデータを削除するようにできます。"
type: origin
...
<p>期限切れのエンティティは、検索結果やクエリ結果には表示されません。ただし、その後のデータ圧縮（24時間以内に行われるはずです）まではストレージに残る可能性があります。</p>

</Admonition>

Zilliz CloudコレクションのTTLプロパティは、秒単位の整数で指定されます。一度設定されると、TTLを超過したデータはコレクションから自動的に削除されます。

削除プロセスは非同期であるため、指定されたTTLが経過した直後に検索結果からデータが削除されない場合があります。代わりに、ガベージコレクション（GC）と圧縮プロセス（非決定的な間隔で発生します）に依存するため、遅延が発生する可能性があります。
...
**1月15日**の**00:00**から**23:59:59**の間に、200億エンティティすべてをマージモードでアップサートし、その後書き込み操作は行いませんでした。

**1月31日**から**2月13日**までの期間、200億エンティティは検索可能であり、クエリカウントは200億のままです。
...
### TTL設定によるデータ失効はいつですか？{#when-does-data-expire-due-to-ttl-settings}
...
データが失効すると、検索結果には含まれなくなります。ただし、クラスターのコンパクションポリシーに従って、その後のシステムコンパクション後にのみ物理的に削除されます。
