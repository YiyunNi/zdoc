...
beta: FALSE
notebook: FALSE
description: "データはデフォルトでコレクションに挿入されたまま残りますが、特定の期間経過後にデータを削除またはクリーンアップしたい場合があります。そのような場合、コレクションの Time-to-Live（TTL）プロパティを設定することで、TTL が期限切れになると Zilliz Cloud が自動的にデータを削除するようにできます。| Cloud"
type: origin
