...
外部ボリュームを作成する前に、[AWS S3 バケット](./integrate-with-aws-s3) または [Google GCS バケット](./integrate-with-gcp) を統合する必要があります。ストレージ統合は、作成する外部ボリュームと同じクラウドプロバイダーおよびリージョンに存在する必要があることに注意してください。
