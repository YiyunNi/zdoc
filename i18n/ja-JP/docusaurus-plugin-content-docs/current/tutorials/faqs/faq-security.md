...
beta: FALSE
notebook: FALSE
description: "このトピックでは、Zilliz Cloud プラットフォームにおけるデータセキュリティに関連する潜在的な問題について説明します。具体的には、認証局、証明書の有効期間、証明書の有効期限を確認する手順、サポートされているトランスポート層セキュリティ (TLS) のバージョン、および認証方法が含まれます。 | CLOUD"
type: origin
