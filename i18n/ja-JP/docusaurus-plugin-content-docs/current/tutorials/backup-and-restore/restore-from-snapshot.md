...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud の復元機能を使用すると、誤ったデータの消失や破損、システム障害などの事態からバックアップファイルを用いてデータを復旧でき、ビジネスの継続性を確保します。インシデントからの復旧、意図しない変更の巻き戻し、テスト用のクラスターのクローン作成などを最小限の影響で実現する信頼性の高い方法です。"
type: origin
