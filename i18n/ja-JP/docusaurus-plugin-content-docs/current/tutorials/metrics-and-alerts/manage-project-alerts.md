...
beta: FALSE
notebook: FALSE
description: "プロジェクトアラートは、指定された条件が満たされた際に通知を送信することで、Zilliz Cloud クラスターの予防的な監視を可能にします。CU 容量やクエリパフォーマンスなどのクラスターメトリクスを監視するようにプロジェクトアラートを構成し、注意が必要な潜在的な問題を即座に検知できます。| Cloud"
type: origin
...
プロジェクトアラートを使用すると、指定された条件が満たされたときに通知を送信することで、Zilliz Cloud クラスターのプロアクティブな監視が可能になります。CU 容量やクエリパフォーマンスなどのクラスターメトリクスを監視するようにプロジェクトアラートを構成でき、注意が必要な潜在的な問題を即座に通知されます。
...
- **組織オーナー** または **プロジェクト管理者** ロールへのアクセス権限
...
<table>
   ...
   <tr>
     <td><p>メトリクスと条件</p></td>
     <td><p>監視パラメータとトリガー設定の組み合わせ表示（例："CU容量 &gt; 80%, Duration &gt;= 10 min"、"Query Latency (P99) &gt; 1000 ms, Duration &gt;= 10 min"）</p></td>
   </tr>
   <tr>
     <td><p>重要度レベル</p></td>
     <td><p>影響度の分類</p><ul><li><p><strong>Warning:</strong> 制限に近づいています</p></li><li><p><strong>Critical:</strong> 即時の対応が必要です</p></li></ul></td>
   </tr>
   <tr>
     <td><p>受信者</p></td>
     <td><p>設定されたメールアドレスや通知チャンネルを含む通知受信者。</p><p>利用可能な通知チャンネルの一覧については、<a href="./manage-notification-channels">通知チャンネルの管理</a>を参照してください。</p></td>
   </tr>
   <tr>
     <td><p>アラート間隔</p></td>
     <td><p>各アラート送信後、設定された時間 동안重複通知を抑制します。</p><ul><li><p>アラートが継続している場合、その間隔中は通知が再送信されません。次の間隔に入る前に通知が再送信されます。</p></li><li><p>アラートが解決されると、アラート間隔はリセットされ、アラート評価が再開されます。</p></li></ul></td>
   </tr>
   <tr>
     <td><p>アクション</p></td>
...
特定のプロジェクト用に作成されたアラートリストを表示できます。パラメータの詳細については、[アラートルールのリスト](/reference/restful/list-alert-rules-v2) を参照してください。
