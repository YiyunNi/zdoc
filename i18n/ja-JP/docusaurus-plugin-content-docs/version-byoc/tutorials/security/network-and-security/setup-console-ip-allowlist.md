...
beta: FALSE
notebook: FALSE
description: "デフォルトでは、組織の Web コンソールはあらゆる IP アドレスからアクセス可能です。アクセスを制限してセキュリティを強化するには、コンソール IP ホワイトリストを設定し、オフィスネットワークの IP など指定されたアドレスからのみ Web コンソールにアクセスできるようにします。| BYOC"
type: origin
