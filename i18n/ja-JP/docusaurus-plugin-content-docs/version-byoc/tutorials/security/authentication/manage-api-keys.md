...
beta: FALSE
notebook: FALSE
description: "API キーは、ユーザーやアプリケーションが Zilliz Cloud のコントロールプレーンおよびデータプレーンのリソースにアクセスするために API または SDK を呼び出す際の認証に使用されます。API キーは名前や ID などの固有のプロパティを持つ英数字の文字列です。| BYOC"
type: origin
