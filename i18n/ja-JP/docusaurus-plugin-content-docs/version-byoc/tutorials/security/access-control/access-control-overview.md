...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は、ロールベースアクセス制御（RBAC）を実装し、Zilliz Cloud 内のリソースへのアクセスを細かく制御します。RBAC（ロールベースアクセス制御）は、ユーザーに直接権限を付与するのではなく、ロールに権限を付与するセキュリティ対策です。リソースに対する特定の権限を含むこれらのロールをユーザーに割り当てることで、ユーザーのアクセス制御を効率的に管理できます。| BYOC"
type: origin
