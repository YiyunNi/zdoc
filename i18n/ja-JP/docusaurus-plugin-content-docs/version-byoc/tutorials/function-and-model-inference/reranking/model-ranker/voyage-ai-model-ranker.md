...
beta: FALSE
notebook: FALSE
description: "Voyage AI Ranker は、Voyage AI の検索アプリケーションを活用します。 | BYOC"
type: origin
...
- リランキングタスクのために特別に訓練されたモデルによる高度なセマンティック理解
...
- **リランキング可能なテキストフィールドを含むコレクションスキーマを計画する**
...
以下のセットアップは、検索とリランキングのためのコレクションとサンプルデータを準備します。
...
Voyage AI Ranker は、コレクションのスキーマの一部としてではなく、**検索時に**定義されます。
...
- クエリとドキュメントがどのように切り捨てられるか、または検証されるか
...
検索とハイブリッド検索では、同じ方法でランカーが適用されます。

どちらの場合も、検索時に `ranker` パラメータを介して再ランク付け関数を渡します。
