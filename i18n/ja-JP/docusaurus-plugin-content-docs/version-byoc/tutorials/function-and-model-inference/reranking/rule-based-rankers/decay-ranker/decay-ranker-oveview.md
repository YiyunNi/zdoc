...
beta: FALSE
notebook: FALSE
description: "従来のベクトル検索では、結果は純粋にベクトル類似度、つまり数学的空間でベクトルがどれだけ密接に一致するかによってランク付けされます。しかし、実際のアプリケーションでは、コンテンツを真に適切にするものは、意味的類似度だけではないことがよくあります。 | BYOC"
type: origin
...
従来のベクトル検索では、結果は純粋にベクトル類似度、つまり数学的空間でベクトルがどれだけ密接に一致するかによってランク付けされます。しかし、実際のアプリケーションでは、コンテンツを真に適切にするものは、意味的類似度だけでは決まらないことがよくあります。
...
- 検索クエリとの類似度がわずかに低い場合でも、トレンド商品をブーストするeコマースプラットフォーム

これらのシナリオはすべて、ベクトル類似度と、時間、距離、人気などの他の数値的要因とのバランスを取るという共通のニーズを共有しています。
...
Decay rankingは、時間や地理的距離などの数値的要因をランキングプロセスに組み込むことで、従来のベクトル検索を強化します。プロセス全体は次の段階に従います。

### ステージ1: 正規化された類似度スコアの計算{#stage-1-calculate-normalized-similarity-scores}

まず、Zilliz Cloudはベクトル類似度スコアを計算し、一貫した比較を確実にするために正規化します。

- **L2**および**JACCARD**距離メトリックの場合（値が低いほど類似度が高いことを示します）: 
...
例えば、ハイブリッド検索で、ある研究論文がベクトル類似度で0.82、BM25ベースのテキスト検索で0.91のスコアを獲得した場合、Zilliz Cloudは減衰係数を適用する前に0.91をベースの類似度スコアとして使用します。
...
減衰再ランキングがなければ、論文Bは純粋なベクトル類似度（0.92）に基づいて最も高い順位になります。しかし、減衰再ランキングを適用すると、次のようになります。

- 論文Cは、類似度が中程度であるにもかかわらず、非常に新しい（昨日発行された）ため、1位に上昇します。

- 論文Bは、類似度が優れているにもかかわらず、比較的新しくないため、3位に下がります。

- 論文DはL2距離（低いほど良い）を使用するため、減衰を適用する前にスコアが1.2から0.76に正規化されます。

## 適切な減衰ランカーの選択\{#choose-the-right-decay-ranker}

Zilliz Cloudは、`gauss`、`exp`、`linear`という異なる減衰ランカーを提供しており、それぞれ特定のユースケース向けに設計されています。
...
<table>
   ...
   <tr>
     ...
     <td><ul><li><p>鮮度が重要なニュースフィード</p></li><li><p>新しいコンテンツが優勢であるべきソーシャルメディア</p></li><li><p>近接性が強く優先されるが、例外的な遠隔地のアイテムも表示されるべき場合</p></li></ul></td>
...
<p>減衰関数を使用する前に、減衰計算に使用する適切な数値フィールド（タイムスタンプ、距離など）を持つcollectionを作成する必要があります。collectionのセットアップ、schema定義、データ挿入を含む完全な動作例については、<a href="./tutorial-implement-time-based-ranking">チュートリアル：Milvusで時間ベースのランキングを実装する</a>を参照してください。</p>
...
<table>
   ...
   <tr>
     ...
     <td><p>はい</p></td>
     <td><p>使用するリランキングメソッドを指定します。</p><p>減衰ランキング機能を有効にするには、<code>"decay"</code>に設定する必要があります。</p></td>
     <td><p><code>"decay"</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.function</code></p></td>
     <td><p>はい</p></td>
     <td><p>適用する数学的減衰ランカーを指定します。関連性低下のカーブ形状を決定します。</p><p>適切な関数を選択するためのガイダンスについては、「<a href="./decay-ranker-oveview#choose-the-right-decay-ranker">適切な減衰ランカーを選択する</a>」セクションを参照してください。</p></td>
   ...
   <tr>
     ...
     <td><p>いいえ</p></td>
     <td><p><code>scale</code>距離でのスコア値で、カーブの傾きを制御します。値が低いほど急な低下カーブになり、値が高いほど緩やかな低下カーブになります。</p><p>0から1の間である必要があります。</p></td>
