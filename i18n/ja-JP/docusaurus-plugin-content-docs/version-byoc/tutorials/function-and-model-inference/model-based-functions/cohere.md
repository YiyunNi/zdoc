...
beta: FALSE
notebook: FALSE
description: "このトピックでは、Milvus で Cohere 埋め込み関数を設定して使用する方法について説明します。 | BYOC"
type: origin
...
以下の例では、テキストデータを保存するための1つのスカラーフィールド `"document"` と、Function モジュールによって生成される埋め込みを保存するための1つのベクトルフィールド `"dense"` を持つスキーマを定義しています。ベクトル次元 (`dim`) は、選択した埋め込みモデルの出力と一致するように設定することを忘れないでください。
