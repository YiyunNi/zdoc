...
beta: FALSE
notebook: FALSE
description: "埋め込みモデルを選択し、テキスト埋め込み関数を持つcollectionを作成することで、Zilliz CloudでOpenAI埋め込みモデルを使用します。 | BYOC"
type: origin
...
- テキスト埋め込み関数が `VARCHAR` フィールド用に生成する密なベクトル埋め込みを保存するために予約されたベクトルフィールド。

以下の例では、テキストデータを保存するための `VARCHAR` フィールド `"document"` と、テキスト埋め込み関数によって生成される密な埋め込みを保存するためのベクトルフィールド `"dense"` を持つスキーマを定義しています。ベクトル次元 (`dim`) を選択した埋め込みモデルの出力に一致するように設定することを忘れないでください。
