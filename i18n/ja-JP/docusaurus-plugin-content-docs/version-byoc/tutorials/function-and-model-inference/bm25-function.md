...
beta: FALSE
notebook: FALSE
description: "BM25 関数は、生のテキストを疎ベクトルに変換し、語彙的な関連性に基づいてドキュメントをスコアリングすることで、全文検索を実現します。この関数は、クエリ用語と密接に一致するテキストドキュメントの効率的な検索をサポートするために、用語ベースのマッチングと頻度を考慮した重み付けを適用します。| BYOC"
type: origin
