...
このガイドでは、Zilliz Cloud で自動バックアップをスケジュールする方法について説明します。オンデマンドバックアップを作成するには、[バックアップの作成](./create-backup) を参照してください。
...
- **バックアップから除外**:
    ...
    - デフォルトユーザー `db_admin` の**パスワード**（[復元](./create-backup) 中に新しいパスワードが生成されます）
...
これはバックアップファイルの作成方法によります。すべての自動バックアップは元のクラスターとともに削除されます。一方、[手動クラスターバックアップ](./create-backup) は永続的に保持され、クラスター削除時にも削除されません。不要になった場合は、手動で削除する必要があります。
