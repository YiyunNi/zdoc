...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud の復元機能を使用すると、データの誤削除、破損、またはシステム障害が発生した場合にバックアップファイルからデータを復旧でき、ビジネスの継続性を確保できます。インシデントからの復旧、意図しない変更の巻き戻し、または最小限の中断でテスト用にクラスタをクローン作成するための信頼性の高い方法です。 | BYOC"
type: origin
