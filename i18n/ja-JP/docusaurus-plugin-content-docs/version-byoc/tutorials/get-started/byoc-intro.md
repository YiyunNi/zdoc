...
beta: CONTACT SALES
notebook: FALSE
description: "Bring Your Own Cloud（BYOC）は、組織が Zilliz Cloud のインフラストラクチャを利用する代わりに、自社のクラウドアカウントでアプリケーションやデータをホストするためのデプロイオプションです。このソリューションは、データの完全な制御権を維持する必要がある特定のセキュリティ要件や規制コンプライアンスのニーズを持つ組織に最適です。| BYOC"
type: origin
