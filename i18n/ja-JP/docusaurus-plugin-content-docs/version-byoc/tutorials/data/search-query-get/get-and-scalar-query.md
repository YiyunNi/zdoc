...
beta: FALSE
notebook: FALSE
description: "ANN 検索に加えて、Zilliz Cloud ではクエリによるメタデータのフィルタリングもサポートしています。このページでは、Query、Get、QueryIterator を使用してエンティティを取得し、メタデータをフィルタリングし、クエリ結果をソートし、スカラー値を集計する方法を紹介します。 | BYOC"
type: origin
