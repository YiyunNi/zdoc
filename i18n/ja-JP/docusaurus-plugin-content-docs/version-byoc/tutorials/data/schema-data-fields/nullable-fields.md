...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud は NULL許容フィールドをサポートしており、フィールド値が欠損している場合や明示的に NULL に設定されていることを許容します。Nullability はスキーマレベルで定義され、データ取り込み、インデックス作成、検索、およびクエリ操作全体で一貫して適用されます。 | BYOC"
type: origin
