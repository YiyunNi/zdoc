...
beta: FALSE
notebook: FALSE
description: "地理情報システム（GIS）、マッピングツール、位置情報サービスなどのアプリケーションを構築する際、幾何データの保存とクエリが必要になることがよくあります。Milvus の `GEOMETRY` データ型は、柔軟な幾何データをネイティブに保存およびクエリするための手段を提供することで、この課題を解決します。| BYOC"
type: origin
