...
beta: FALSE
notebook: FALSE
description: "特に IoT（モノのインターネット）や自動運転分野における最新の AI アプリケーションでは、タイムスタンプとベクトル埋め込みを含むセンサー測定値、エラーコードと音声スニペットを含む診断ログ、あるいは位置情報、速度、シーンコンテキストを含む移動セグメントなど、リッチで構造化されたイベントに対して推論を行うのが一般的です。これらを実現するには、データベースがネストされたデータの取り込みと検索をネイティブにサポートする必要があります。 | BYOC"
type: origin
