...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud では、エイリアスはコレクションの二次的で変更可能な名前です。エイリアスを使用することで、アプリケーションコードを変更せずにコレクションを動的に切り替えることができる抽象化レイヤーが提供されます。これは、本番環境におけるシームレスなデータ更新、A/B テスト、その他の運用タスクに特に役立ちます。| BYOC"
type: origin
