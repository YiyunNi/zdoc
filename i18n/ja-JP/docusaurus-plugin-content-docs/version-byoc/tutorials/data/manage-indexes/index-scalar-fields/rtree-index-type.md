...
beta: FALSE
notebook: FALSE
description: "`RTREE` インデックスは、Zilliz Cloud の `GEOMETRY` フィールドに対するクエリを高速化するツリー構造のデータ構造です。Well-known text (WKT) 形式で点、線、ポリゴンなどの幾何オブジェクトを格納するコレクションがあり、空間フィルタリングを高速化したい場合、`RTREE` は最適な選択です。 | BYOC"
type: origin
