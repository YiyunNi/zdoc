...
beta: FALSE
notebook: FALSE
description: "ビットマップインデックスは、カーディナリティの低いスカラーフィールドにおけるクエリパフォーマンスを向上させるために設計された効率的なインデックス手法です。カーディナリティとは、フィールド内の固有値の数を指します。固有要素が少ないフィールドは、低カーディナリティとみなされます。| BYOC"
type: origin
