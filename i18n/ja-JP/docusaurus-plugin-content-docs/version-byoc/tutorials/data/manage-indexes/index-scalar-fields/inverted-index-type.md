...
beta: FALSE
notebook: FALSE
description: "データに対して頻繁にフィルタークエリを実行する必要がある場合、転置インデックスを使用することでクエリパフォーマンスを大幅に向上させることができます。すべてのドキュメントをスキャンするのではなく、Zilliz Cloud は転置インデックスを使用して、フィルター条件に一致する正確なレコードを迅速に特定します。 | BYOC"
type: origin
