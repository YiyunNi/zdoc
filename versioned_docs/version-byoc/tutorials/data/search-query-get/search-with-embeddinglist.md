...
- $[e_{d1}, e_{d2}, \dots, e_{dn}] ∈ \R^{n \times d}$: the number of vector embeddings in the embedding list representing the document is within the range of $\R^{n \times d}$.

- $[e_{q1}, e_{q2}, \dots, e_{qm}] ∈ \R^{m \times d}$: the number of vector embeddings in the embedding list representing the query is within the range of $\R^{m \times d}$.
...
During data ingestion, a document page, represented as a raw image, is divided into multiple visual patches, each of which is embedded to generate a list of vector embeddings. Then they are projected into the language model's vector space to obtain the final embedding list, as in $d \rightarrow E_d = [e_{d1}, e_{d2}, \dots, e_{dn}] ∈ \R^{n \times d}$. When a query arrives, it is tokenized, and each token is embedded to generate a list of vector embeddings, as in $q \rightarrow E_q = [e_{q1}, e_{q2}, \dots, e_{qm}] ∈ \R^{m \times d}$. Then, **MAX_SIM** has been applied to compare the two embedding lists and obtain the final score between the query and the document page. 
