...
- **Offline preference**: For batch workloads, consider using an On-demand Compute database instead of a Serving Cluster. Database endpoints use on-demand CUs and do not affect online services.
...
- The topK limit is always constrained to approximately **one-tenth of the segment size**. Even with Large TopK enabled, you cannot retrieve an entire collection in a single query.
...
- Three cluster types (Performance-optimized, Capacity-optimized, and Tiered Storage) all support Large TopK. The only difference is where data resides (all-memory vs. NVMe vs. object storage).

## FAQ\{#faq}

**Q: Can I switch back and forth frequently?**

Technically, yes, but it is not recommended. Each switch requires releasing, dropping, and recreating the index, during which search is unavailable. In offline scenarios, each rebuild also incurs Index Build CU charges.

**Q: Why is query_mode a collection property instead of an index parameter?**

Large TopK affects more than just the index algorithm. It changes the segment size strategy (from ~1 million rows to ~10 million rows), which is a collection-level data organization decision. Since all indexes on a collection share the same segments, the query mode must be set at the collection level.

**Q: Does enabling Large TopK increase storage costs?**

No. In fact, Large TopK uses deeper compression (RaBitQ), so the vector portion of your data may consume less storage than the default mode. However, disk quota limits still apply.