...
notebook: FALSE
description: "The `synonym` filter rewrites tokens according to a synonym dictionary, so that related terms match during search. It supports two modes of operation and two ways of supplying the dictionary | BYOC"
type: origin
...
<table>
   ...
   <tr>
     ...
     <td><p>A reference to a <a href="./manage-file-resources">file resource</a> that stores synonym rules, one per line. Use for larger dictionaries. See <a href="./synonym-filter#external-dictionary-file">External dictionary file</a> below.</p></td>
     <td><p>—</p></td>
   </tr>
   <tr>
...
### `expand: true` — keep the original, add synonyms\{#expand-true-keep-the-original-add-synonyms}
...
### `expand: false` — rewrite to canonical form\{#expand-false-rewrite-to-canonical-form}
