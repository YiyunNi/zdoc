---
title: "Release Notes (Nov 18, 2022) | BYOC"
...
notebook: FALSE
description: "We're excited to announce the general availability of a major Zilliz Cloud update, now open to the public without the need for an invitation. This release introduces capacity-optimized Compute Units (CUs), allowing up to 32 CUs per database for enhanced performance. It also enables data import from local files and AWS S3 buckets, simplifies the process with auto-indexing, and adds resource monitoring for QPS and query latency. Additionally, database creation speed has been improved by five times, and the user interface has been optimized for a superior experience. | BYOC"
type: origin
