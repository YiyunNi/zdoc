...
beta: FALSE
notebook: FALSE
description: "The `RTREE` index is a tree-based data structure that accelerates queries on `GEOMETRY` fields in Zilliz Cloud. If your collection stores geometric objects such as points, lines, or polygans in Well-known text (WKT) format and you want to accelerate spatial filtering, `RTREE` is an ideal choice. | Cloud"
type: origin
