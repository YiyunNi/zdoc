...
In a global cluster, both the primary and secondary clusters are billed as regular Zilliz Cloud [Dedicated clusters](./storage-cost) for their compute and storage usage, plus additional [data transfer](./data-transfer-cost_1) charges for data replication between clusters. 
...
- **Project plan availability**: You need to have a multi-regional project on the Business Critical plan to access the global cluster feature. In addition, secondary cluster regions in a Global Cluster are limited to the regions supported by your project.
