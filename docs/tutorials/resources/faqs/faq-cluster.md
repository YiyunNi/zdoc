...
If your data exceeds the maximum capacity of a free cluster, please [upgrade](./select-zilliz-cloud-service-plans) to the Serverless or Dedicated deployment option to create a new cluster and [migrate your data](./offline-migration) there. For more information about the capacity of a  cluster, please refer to [Select the Right CU](./select-the-right-cluster-type#assess-capacity)

### What can I do if I receive the error "quota exceeded\[reason=disk quota exceeded, please allocate more resources"?{#what-can-i-do-if-i-receive-the-error-quota-exceededreasondisk-quota-exceeded-please-allocate-more-resources}

When inserting or upserting data, you will receive this error because your data exceeds the serving cluster CU capacity. A free cluster can handle 1 million 768-dimensional vectors. The capacity of a dedicated cluster depends on its [cluster type and CU size](./select-the-right-cluster-type#assess-capacity).
...
1. Create a new serving cluster with the desired cluster type. Use the [calculator](https://zilliz.com/pricing#calculator) to determine the query CU of this new serving cluster.
