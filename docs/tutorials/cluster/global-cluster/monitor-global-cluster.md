...
<table>
   ...
   <tr>
     <td><p>FENCED</p></td>
     <td><p>After a switchover or failover, the original primary cluster is fenced and rejects all write requests.</p></td>
     <td><p>Wait for the operation to complete. Do not initiate additional switchovers or failovers.</p></td>
   </tr>
   <tr>
     <td><p>REBUILDING</p></td>
     <td><p>A secondary cluster is re-syncing data from the new primary after a restore or failover.</p></td>
     <td><p>Wait for rebuilding to complete. Reads may be temporarily unavailable on the rebuilding cluster.</p></td>
   </tr>
   <tr>
     <td><p>REBUILD_FAILED</p></td>
     <td><p>A secondary cluster rebuild has failed.</p></td>
     <td><p>Retry the rebuild operation. <a href="http://support.zilliz.com/">Contact support</a> if the issue persists.</p></td>
   </tr>
</table>
