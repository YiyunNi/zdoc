...
- The same [resource limits](./limits#cus) apply as for regular Dedicated clusters (e.g., CU × Replica ≤ 1,024).

- During CU scaling, the cluster status changes to Modifying. [Switchover](./switchover-and-failover#perform-a-switchover) is blocked while scaling is in progress.

- [Failover](./switchover-and-failover#perform-a-failover) can still be triggered during CU scaling as an emergency operation, but the scaling task will fail and be retried after failover completes.

## Scale replicas\{#scale-replicas}

Replica scaling is controlled independently per cluster. Each cluster in the global cluster — primary and secondary — can have a different replica count tailored to its regional workload. This allows you to allocate more resources in high-traffic regions without over-provisioning in others. The following is an example of configuration replicas for each cluster.
