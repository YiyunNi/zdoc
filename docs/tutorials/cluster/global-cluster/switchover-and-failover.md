...
```plaintext
               ...
               └──┬─────────┬────┘   └──┬─────────┬───┘
                  ...
                  │         │
                  │         ▼
                  │     ABNORMAL ──────────── Auto-recovery ──► RUNNING
                  │         │
                  │      Failover
                  │         │
                  │         ▼
                  │    ┌──────────────────────────────────────┐
                  │    │  Target secondary ──► New Primary    │
                  │    │  Old primary ──► Discarded (recycle) │
                  │    │  System auto-creates new secondary   │
                  │    └──────────────────────┬───────────────┘
                  │                           │
                  │                           ▼
                  │                       CREATING
...
- **Failover**:
    ...
    - After the failover completes, Zilliz Cloud automatically creates a new secondary cluster to restore the full topology. The new secondary starts in **CREATING** status and transitions to **RUNNING** once provisioning and data sync are complete. If creation fails, the cluster enters **REBUILD_FAILED** status. You can retry the rebuild or [contact us](http://support.zilliz.com) for assistance.
